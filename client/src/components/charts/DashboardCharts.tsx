...
export const DoughnutChart: React.FC<ChartComponentProps> = ({
    data,
    options,
    height = 300,
}) => {
...
// ✅ Utility Functions
export const formatMonthlyTrendsData = (
    monthlyData: any[],
    title: string = "Monthly Trends"
) => {
    const months = [
        "Jan",
        "Feb",
...
export const formatStatusDistributionData = (statusData: any[]) => {
    ...
    const backgroundColor = statusData.map(
        (item) =>
            colors[item._id as keyof typeof colors] || "rgb(156, 163, 175)"
    );
...
export const formatLeaveTypeData = (typeData: any[]) => {
    ...
    const backgroundColor = typeData.map(
        (item) =>
            colors[item._id as keyof typeof colors] || "rgb(156, 163, 175)"
    );
